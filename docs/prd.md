...
- Licensing/access verification for the pretrained datasets named in ai_pipelines.md §7 (sourcing itself is already researched and captured there — originally recovered from `prometheus-1`'s design work, now archived externally, not in this repo, §15 — but each dataset's license still needs confirming before fine-tuning against it).
...
| `server/core/config.py`'s `CORS_ORIGINS` default fixed to include `client/`'s real dev port (5173) | Caught by actually running client + server together (frontend.md §5's verification pass): the default still only allowed port 3000 (the archived Next.js app's port), so every real `/api/*` call from the actual current client was silently rejected by the browser's own CORS preflight and the app fell back to mock mode — indistinguishable at a glance from "the backend isn't running", which it was |
| `client/`'s `.officer-avatar` (Topbar) renamed to `.topbar-avatar` | This app has no CSS scoping, and `LoginScreen.css` already defined an unrelated `.officer-avatar` (its own 82px post-login-card avatar) that was silently winning the cascade, blowing the topbar badge up to 82px and overlapping/hiding the accessibility bar's controls on every page — another bug only a real running browser surfaced |
| `contrib/` removed from the repo entirely (2026-09-14), not just left archived in-tree | Its owner (Dhiraj) had both subtrees — `contrib/aneesh/backend/` (rejected-as-production backend, decision above) and `contrib/nextjs-frontend-archive/` (superseded Next.js frontend, decision above) — zipped and stored externally before asking for the in-repo copy to go. Everything either subtree offered that was still actionable had already been extracted first: `server/` was ported from and has since fully superseded `contrib/aneesh/backend/`'s capabilities (backend.md §12), and `client/` had already pulled the two real, still-useful pieces out of the Next.js archive (a working `AccessibilityBar.jsx`, a real live-event feed) before it left — frontend.md §6 names the two pieces that weren't ported first (a WHEP/HLS player pattern, a gap-analysis panel UI), for whoever holds the external archive to pull from later if needed. Nothing in `server/` or `client/` depends on `contrib/` still being present — both were verified working (63 backend tests, a full Playwright pass across every client page) before this removal |
| `CameraMap.jsx` rewired to real `api.getCameras()` data (third pass, same day) rather than left as the documented known gap | The team explicitly asked for this specific item next. All fabricated content was removed rather than left disclaimed: the fixed "48 Nodes Online / 77.1% Coverage" stat strip, a fake AI bounding-box overlay with an invented license plate ("VEHICLE [GJ-05-AB-7104] 98%"), a fake ticking "LIVE FEED" timestamp, a "Copy RTSP" button that copied a URL to a host that was never real (`rtsp_url` isn't exposed to the frontend, by design — a security boundary, not an oversight), and a "Snapshot" button with no real capture endpoint behind it. Replaced with real per-camera fields the backend actually has (protocol_type, ai_profile, onboarding_source) and one real action, "Check Adapter Health", against the same real endpoint `SystemNetwork.jsx` uses — verified via backend logs that it genuinely opens a live RTSP connection attempt, not a canned response |
| Model 1's gap-analysis report surfaced in `CameraRegistry.jsx` rather than a new standalone page | The real endpoint (`GET /api/cameras/gap-analysis`) and client method (`api.getGapAnalysis()`) already existed and had simply never been rendered anywhere — adding a panel to the existing registry page (where an operator would look for it) closes the gap with no new route or nav item needed |
| New `SystemNetwork.jsx` page (`/system`) added rather than folding pipeline/adapter health into an existing page | prd.md's original page-count discipline (§15's "exactly 8 top-level pages" decision) was written before this specific requirement (pipeline health, per-camera adapter health, an integration log) was scoped out — it doesn't fit naturally inside any of the 8 without overloading one. Built honestly per-tab: Pipeline Health and Adapter Health are real (`GET /api/health/`, `GET /api/cameras/{uid}/adapter/health`); the Integration Log tab states plainly that no such endpoint exists on the backend yet rather than fabricating rows, after checking every router first |
| Adapter health in `SystemNetwork.jsx` checked on-demand per selected camera, never looped automatically across the fleet | The real endpoint genuinely opens a live network connection per call (its own docstring says so, backend.md) — auto-checking every registered camera just to render a status page would mean silently opening N live connections as a side effect of a page load, which is a different (and worse) thing than what an operator asking to check one camera intends |
| Real HLS playback wired into `LiveCameras.jsx` via `hls.js` and the existing `POST /api/streams/{id}/start` endpoint, left genuinely failing in this sandbox rather than mocked to look connected | This sandbox has no ffmpeg installed and no `rtsp_url` pointing at a real reachable camera, so the real code path genuinely fails (a real 500 from the backend's real attempt to spawn `ffprobe`) — shown honestly as an error with a Retry button. Building a fake "success" state here would be indistinguishable from a real stream to anyone demoing it, which is exactly the failure mode this project's honesty conventions exist to prevent; the failure itself is proof the feature is real, not mocked, and it plays real video given a real camera and a host with ffmpeg installed |
| Full UI translation scoped to app shell + every page's `<h1>`/subtitle (`src/i18n/`), not every label on every page | The highest-visibility text an operator sees on every screen, on every navigation; scoped and stated explicitly in `translations.js`'s own header comment rather than silently leaving gaps. Dynamic data (camera names, alert descriptions — real database content) is deliberately left untranslated, since machine-retranslating real record data would misrepresent what's actually stored. Widening the scope (data-table headers, modal/form labels) is a mechanical follow-up, not a design question, whenever someone wants to do it |
