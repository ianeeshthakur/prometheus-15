...
- [x] Error reporting capturing camera id, exact URL, client + version, UTC timestamp, and client-side error log — closed 2026-09-14 (§12.9). Real, queryable rows now, not just plain `logger` calls.
...
### 12.9 Structured error reporting closed (sixth pass, 2026-09-14)

The only §2 checklist item that was actually closeable by writing code, of the ones open at the time — the other five in §2/§12.7 are genuinely blocked on external resources this environment doesn't have (a real camera feed, real OCR error-rate data, a trained re-id model, an organizer-published ingest host, and a deliberately-deferred Redis-vs-Kafka decision). "Error reporting capturing camera id, exact URL, client + version, UTC timestamp, and client-side error log" had none of those blockers — it was just never built, plain `logger.error()` calls standing in for it.

Built:
- [x] `models/event.py`'s `AdapterErrorLog` + `services/error_log_service.py` — a real, queryable error log. `redact_stream_url()` strips credentials from any RTSP/HLS URL before it's ever written to the DB (same boundary `CameraResponse` already enforces for the registry — host/path kept for real debugging value, credentials never stored), unit-tested against both the `user:pass@` form and query-string tokens.
- [x] Wired into every real adapter failure point that used to be just a `logger.error()` call: `adapters/rtsp.py`/`hls.py`'s connect failures and reconnect-exhausted case, and `video/ffmpeg_runner.py`'s probe failure, start failure, and unexpected-exit cases. `client_version` is genuinely detected, never guessed — `cv2.__version__` for the OpenCV paths, a real `ffmpeg -version` subprocess call for the FFmpeg path, which honestly reports `"not installed"` in this sandbox (matching `GET /api/health/`'s own `ffmpeg_available` flag) rather than a hardcoded version string.
- [x] The "client-side error log" half — `POST /api/streams/client-errors` (auth-required), and a real caller: `client/src/components/HlsPlayer.jsx`'s existing `onError` callback now also reports the real playback failure through `client/src/pages/LiveCameras.jsx`, with the real `hls.js` version (`Hls.version`), best-effort (a failed report never surfaces as a second error to the user).
- [x] `GET /api/admin/adapter-errors` (admin-only, optional `camera_uid` filter) to actually query the log — same reasoning as the audit log: real (redacted) stream URLs and camera identifiers shouldn't be visible to a non-admin or unauthenticated caller.

Caught a real, separate bug while building this: `GET /api/admin/audit-log` returned `{"entries": entries}` with raw ORM objects and no `response_model` — bypassing `schemas/common.py`'s `UtcDatetime` entirely and silently reproducing the exact "timestamps off by the server's UTC offset" bug already fixed everywhere else (commit `ced635d`). `AuditLogResponse` (`schemas/user.py`) existed but had never actually been wired in. Fixed both `audit-log` and the new `adapter-errors` endpoint to validate through their real Pydantic response models.

Verified end-to-end against the real running server: created a real camera with a real (unreachable) `rtsp_url`, called `POST /api/streams/{id}/start`, watched it genuinely fail (`ffprobe` isn't installed here — `[WinError 2]`), and confirmed a real row appeared in `GET /api/admin/adapter-errors` with credentials redacted, a real detected `client_version` ("not installed", honestly), and a real UTC timestamp ending in `Z`. Also called `POST /api/streams/client-errors` directly and confirmed it appears in the same log with `source: CLIENT`. 10 new tests (`tests/test_error_logging.py`); 78/78 pytest tests pass total.
