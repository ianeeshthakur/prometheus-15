...
57 tests, all passing as of 2026-09-13 (§12.7). DB isolation is at the module (test-file) level, not per-function — an autouse fixture wipes operational tables between files (`tests/conftest.py`), closing the real risk (cross-file contamination) without the full per-function `db/database.py` engine/session restructuring a true per-test DB would need. Verified order-independent by running file subsets out of default collection order.
...
### 12.7 Remaining known gaps — tooling built, the actual blockers unchanged (2026-09-13, 57 passing pytest tests)
