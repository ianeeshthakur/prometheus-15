...
def test_create_watchlist_entry(client):
    ...
    resp = client.post("/api/watchlists/", json=entry)
    assert resp.status_code == 201, resp.text
    assert resp.json()["match_count"] == 0


def test_list_watchlist_includes_created_entry(client):
    resp = client.get("/api/watchlists/")
    assert resp.status_code == 200
    assert any(e["identifier"] == "GJ01AB4521" for e in resp.json())


def test_matching_plate_produces_real_alert_and_increments_match_count(client):
    ...
    entry_id = next(e["id"] for e in client.get("/api/watchlists/").json() if e["identifier"] == "GJ01AB4521")
    ...
    entry_after = next(e for e in client.get("/api/watchlists/").json() if e["id"] == entry_id)
